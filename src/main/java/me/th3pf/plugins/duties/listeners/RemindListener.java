...
public class RemindListener implements Listener
{
   ...
   public void onPlayerInteract( PlayerInteractEvent event )
   {
      ...
      if( Duties.Config.GetInteger( "ReminderCooldown" ).longValue() != 0 )
         if( ( event.getPlayer().getWorld().getTime() - Duties.LastChestReminderTime.get( event.getPlayer().getName() ) ) <= Duties.Config.GetInteger( "ReminderCooldown" ).longValue() )
         {
            return;
         }
   ...
   public void onPlayerDropItem( PlayerDropItemEvent event )
   {
      ...
      if( Duties.Config.GetInteger( "ReminderCooldown" ).longValue() != 0 )
         if( ( event.getPlayer().getWorld().getTime() - Duties.LastDropReminderTime.get( event.getPlayer().getName() ) ) <= Duties.Config.GetInteger( "ReminderCooldown" ).longValue() )
         {
            return;
         }
